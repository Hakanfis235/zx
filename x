...
local Library do 
    ...
    Library.Theme = TableClone(Themes["Fatality"])
    ...
    if IsMobile then
        ...
        local MenuButton = Instances:Create("TextButton", {
            ...
            AutoButtonColor = false,
            ZIndex = 1000
        }) MenuButton:AddToTheme({BackgroundColor3 = "Element"})
        ...
        }) MenuIcon:AddToTheme({ImageColor3 = "Accent"})
        ...
        local LockButton = Instances:Create("TextButton", {
            ...
            AutoButtonColor = false,
            ZIndex = 1000
        }) LockButton:AddToTheme({BackgroundColor3 = "Element"})
        ...
        local LockIconData = Library.GetIcon("lock") or Library.GetIcon("lock-open")
        local LockIcon = Instances:Create("ImageLabel", {
            ...
            Position = UDim2New(0.5, 0, 0.5, 0),
            Size = UDim2New(0, 24, 0, 24),
            Image = LockIconData and LockIconData.Url or "rbxassetid://15117261700",
            ...
            ImageColor3 = FromRGB(100, 255, 100)
        })

        local function UpdateLockState()
            if Library.MenuLocked then
                LockIcon.Instance.ImageColor3 = FromRGB(255, 100, 100)
                ...
                LockIcon.Instance.ImageColor3 = FromRGB(100, 255, 100)
                if Library.CurrentWindow and Library.CurrentWindow.Items and Library.CurrentWindow.Items["MainFrame"] then
        ...
        MenuButton.Instance.InputBegan:Connect(function(input)
            if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                ...
                MenuStartPos = MenuButton.Instance.Position
                
                input.Changed:Connect(function()
                    if input.UserInputState == Enum.UserInputState.End then
                        if MenuDragging and (input.Position - MenuDragStart).Magnitude < 5 then
        ...
        LockButton.Instance.InputBegan:Connect(function(input)
            if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                ...
                LockStartPos = LockButton.Instance.Position
                
                input.Changed:Connect(function()
                    if input.UserInputState == Enum.UserInputState.End then
                        if LockDragging and (input.Position - LockDragStart).Magnitude < 5 then
            ...
            if LockDragging and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
                ...
                if delta.Magnitude > 5 then
                    LockButton.Instance.Position = UDim2New(
                        LockStartPos.X.Scale,
                        LockStartPos.X.Offset + delta.X,
                        LockStartPos.Y.Scale,
                        LockStartPos.Y.Offset + delta.Y
                    )
                end
            end
        end)
...
local Window = Library:Window({
    Name = "Airflow",
    SubTitle = "dsc.gg/airflowscript",
    TimeRemaining = "67 years",
    Icon = 103859712365480
})
...
local SectionLeft = HomePage:Section({Name = "Examples", Icon = "box", Side = 1})
local SectionRight = HomePage:Section({Name = "More Examples", Icon = "package", Side = 2})
...
local ToggleWithColorpicker = SectionLeft:Toggle({
...
local ToggleWithKeybind = SectionLeft:Toggle({
...
SectionLeft:Button({
    ...
    Callback = function()
        Library:Notification("This is a notification notifying..", "bell", 5)
    end
})
...
SectionRight:Label("Label with Colorpicker"):Colorpicker({
    Flag = "Colorpicker",
...
SectionRight:Label("Label with Keybind"):Keybind({
    Flag = "Keybind",
...
local LabelWithBoth = SectionRight:Label("Colorpicker + Keybind")
LabelWithBoth:Colorpicker({
    Flag = "ColorpickerBoth",
    Default = Color3.fromRGB(127, 96, 201),
    Callback = function(Value)
        print(Value)
    end
})
LabelWithBoth:Keybind({
    Flag = "KeybindBoth",
...
SectionLeft:SearchableDropdown({
    Name = "Searchable Dropdown",
    Flag = "SearchableDropdown",
    Items = {"Apple", "Apricot", "Banana", "Blueberry", "Cherry", "Cranberry", "Dragon Fruit", "Durian", "Elderberry", "Fig", "Grape", "Grapefruit", "Honeydew", "Ice Cream Bean", "Jackfruit", "Kiwi", "Lemon", "Lime", "Mango", "Nectarine", "Orange", "Papaya", "Peach", "Pear", "Pineapple", "Plum", "Raspberry", "Strawberry", "Tangerine", "Watermelon"},
    Default = "Apple",
    MaxSize = 150,
    Callback = function(Value)
...
SectionLeft:SearchableDropdown({
    Name = "Multi Search Dropdown",
    Flag = "MultiSearchDropdown",
    Multi = true,
    Items = {"Red", "Orange", "Yellow", "Green", "Blue", "Indigo", "Violet", "Pink", "Purple", "Brown", "Black", "White", "Gray", "Cyan", "Magenta", "Lime", "Navy", "Teal", "Silver", "Gold"},
    Default = {"Red", "Blue"},
    MaxSize = 150,
    Callback = function(Value)
        print("Multi selected:", Value)
    end
})
